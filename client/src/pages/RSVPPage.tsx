...
const RSVPPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchGuestData = async () => {
      ...
      try {
        const response = await axios.get(`http://localhost:5000/api/rsvp/${guestId}`);
        const guestData = response.data;
        setGuest(guestData);
        
        // Fetch awards if they exist
        if (guestData.rsvp_status === 'yes') {
          const awardsResponse = await axios.get(`http://localhost:5000/api/events/${guestData.event_id}/awards`);
  ...
  const submitRSVP = async (response: 'yes' | 'no') => {
    ...
    setSubmitting(true);
    try {
      await axios.post(`http://localhost:5000/api/rsvp/${guestId}`, { response });
      ...
      if (response === 'yes' && guest?.event_id) {
        const awardsResponse = await axios.get(`http://localhost:5000/api/events/${guest.event_id}/awards`);
        setAwards(awardsResponse.data);
      }
    } catch (error) {
  ...
  const submitVote = async (awardId: string, nomineeId: string) => {
    ...
    try {
      await axios.post(`http://localhost:5000/api/events/${guest.event_id}/vote`, {
        awardId,
        voterId: guest.id,
        nomineeId
      });
