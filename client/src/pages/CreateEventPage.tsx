    ...
    try {
      ...
      for (const guest of guests) {
        if (guest.firstName && guest.lastName) {
          await axios.post(`http://localhost:5000/api/events/${newEventId}/guests`, {
      ...
      for (const award of awards) {
        if (award.title) {
          await axios.post(`http://localhost:5000/api/events/${newEventId}/awards`, {
            title: award.title,
      ...
      navigate(`/event/${newEventId}/dashboard`);
    } catch (error) {
